 ...
 *
 * @param {number} artistiName Poistettavan tuoterivin id
 */
async function removeArtisti(artistiName) {
	let success = await removeFromServer(artistiName);

	if (success) {
		removeFromPage(artistiName);
	}
}
